...
- 도구는 2개만 제공: (1) `search`(검색 결과 링크 본문 수집 + 출처 기반 답변 생성), (2) `transform`(입력 텍스트 포맷 변환).
...
- 운영 가능한 최소 기준(로그/트레이스/오류)을 포함한다. LangGraph는 LangSmith 연동을 통해 트레이스 기반 디버깅/관측을 지원한다. citeturn4view0turn4view2
...
- 운영 메트릭 수집: 요청 지연시간, 툴 호출 횟수, 실패율을 로그로 적재한다.
...
- **집행**:  
  - `DIRECT_ANSWER` → 답변 생성  
  - `CALL_TOOL` → 검색/변환 도구 실행. 검색인 경우 링크 본문 수집 후 본문 기반 최종 답변 생성  
  - `ASK_CLARIFY` → 추가 질문  
  - `REFUSE` → 거절 + 대안 제시  
...
sequenceDiagram
  ...
  alt nextAction = DIRECT_ANSWER
    ...
    G->>G: allowedTools 기반 단일 도구 선택
    G->>T: pendingTool 실행
    T->>S: fetch /api/tools/* 또는 provider 호출
    S-->>T: tool result
    alt search
      T->>S: 각 링크 본문 수집
      T-->>G: search result + bodyText
      G->>M: 본문 기반 최종 답변 생성
      M-->>G: AIMessage
    else transform
      T-->>G: transformed text
    end
...
- **페이지**: `/chat/[sessionId]`
  - 상단: 고정 제목 + 세션 정보(“학습 맥락 요약 보기” 토글)
  - 중앙: 메시지 리스트(유저/어시스턴트/툴 로그는 기본 숨김, 디버그 모드에서 펼침)
  - 하단: 입력창 + 전송 버튼 + “근거 필요” 체크 옵션(선택)
  - 우측(옵션): 실행 트레이스 패널(`nextAction`, `reasonSummary`, `allowedTools`, `toolExecutions`)
- **UX 원칙**
  - 스트리밍 중에는 “생성 중…” 배지 표시
...
- `Message`: 세션별 N개(현재 구현 저장 기준 `user|ai`)
- `ToolExecution`: tool call 기록(이름, args, 성공/실패, 지연시간)
- `Trace`: (옵션) 트레이스 메타데이터; LangSmith 사용 시 외부로도 전송 가능 citeturn4view2turn4view0
...
- `search` 실행 후 후처리
  - 검색 결과 URL 본문을 수집한다.
  - 수집 본문을 바탕으로 교육적으로 정리된 최종 응답을 생성한다.
  - 모델 제공자 오류가 발생하면 제목 목록 기반의 축약 응답으로 fallback할 수 있다.
  - 최종 응답과 별도로 `sources`를 사용자에게 제공한다.
...
- Response
  ...
  - 구현 기준: 모델이 생성하는 token delta를 `event: token`으로 즉시 전달하고, 완료 시 `event: message`에 최종 완성 텍스트를 싣는다.
...
현재 구현은 모델 raw `tool_calls` 루프가 아니라, 서버가 `RouteDecision.allowedTools`와 사용자 입력을 바탕으로 단일 도구를 결정적으로 선택하는 구조다. `callModelWithTools`는 이름과 달리 "도구 허용 범위를 바탕으로 `pendingTool`을 정하는 서버 노드"이고, `toolNode`는 선택된 도구를 실행한 뒤 필요 시 답변 생성 노드로 이어진다.
...
- `callModelWithTools`: `allowedTools`와 사용자 입력을 바탕으로 `pendingTool` 결정
- `toolNode`: 선택된 단일 도구 실행
...
- tool loop:
  - `callModelWithTools`가 `pendingTool`을 반환하면 `toolNode`로 이동
  - `toolNode` 실행 후 `shouldRetry=true`면 `callModelWithTools`로 재진입
  - 현재 구현의 최대 재시도 한도는 2회다.

**오류/폴백 처리**

- `toolNode` 실행 실패(타임아웃/검증 실패/5xx)
  - 검색 실패는 재시도 가능 상태이면 루프를 한 번 더 돌리고, 아니면 `ASK_CLARIFY` 또는 안전한 fallback 응답으로 종료한다.
- 라우터 구조화 출력 파싱 실패
...
- (App Router 기준)
  ...
  - `app/chat/[sessionId]/ChatClient.tsx`: 메시지 리스트, SSE 수신, 입력창, trace 토글을 포함한 클라이언트 UI
  - `app/chat/[sessionId]/chat.css`: 채팅 화면 스타일
...
import { createTurnId } from "@/core/id/ids";
import { createSseResponse } from "@/presentation/chat/sse";
      ...
      (async () => {
        try {
          const result = await runChatGraph(deps, {
            ...
            turnId,
            userMessage: message,
            needsSources: clientOptions?.needsSources ?? false,
            debug: clientOptions?.debug ?? false,
          });

          send("message", { text: result.finalText, nextAction: result.finalNextAction });
          send("done", { ok: true, latencyMs: result.latencyMs });
        } catch (err: any) {
...
현재 구현은 LangGraph checkpointer의 `thread_id`를 직접 쓰지 않고, `runChatGraph(...)`에 `sessionId`, `requestId`, `turnId`를 명시적으로 전달한다.
...
### 코드 스니펫: LangGraph 그래프 스켈레톤(서버 결정적 도구 선택)

```ts
// src/application/graph/chatGraph.ts 개념 요약
const planNextAction = async (state) => {
  const routeDecision = await llmPort.planNextAction({
...
const callModelWithTools = async (state) => {
  return {
    pendingTool: chooseTool(state.routeDecision, state.userMessage),
    shouldRetry: false,
  };
};
...
const toolNode = async (state) => {
  if (state.pendingTool.toolName === "search") {
    const result = await searchPort.search(state.pendingTool.args);
    const finalText = await llmPort.generateSearchAnswer({
      userMessage: state.userMessage,
      masterContext: state.masterContext,
      items: result.items,
      onToken: emitToken,
    ...
    return {
      finalText,
      sources: normalizeSources(result.items),
      finalNextAction: "CALL_TOOL",
    };
  }

  const result = await transformPort.transform(state.pendingTool.args);
  return {
...
- LangGraph는 상태/분기/루프를 구성하는 데 사용하지만, 이 MVP는 도구 선택을 모델 native tool call에 위임하지 않는다.
- 실제 구현의 도구 실행 책임은 서버 애플리케이션 계층에 있으며, 모델은 `planNextAction`과 최종 답변 생성에만 관여한다.
...
- **메트릭**
  - 요청당: latency, tool call 횟수, tool 실패율, 라우팅 분포(NextAction 비율)
  - 현재 구현은 비용/토큰 사용량을 별도 저장하거나 UI에 노출하지 않는다.
- **레이트 리밋/쿼터 대응**
...
- **Week B**
  ...
  - `search`, `transform` tool + 서버 결정적 tool loop 구성
  - `/api/chat` 스트리밍 응답(SSE) 연결(OpenAI도 SSE 기반 streaming 가이드 제공) citeturn3search0turn1search0
- **Week C**
  - 로그/트레이스 기록 + (옵션) LangSmith 연동 citeturn4view2turn4view0
